...
rem    Linux e macOS .. curl -fsSL https://raw.githubusercontent.com/alexkads/VintageLightbox/dev/scripts/instalar-vintagelightbox-tauri.cmd | sh
rem
...
#    `python3 scripts/gerar-instaladores.py`, junto com o do outro app
#    (instalar-vintagelightbox-gpui.cmd). Edite o modelo e gere de novo: o teste
#    `scripts/testar-instalador.py` recusa um arquivo que divirja do modelo.
#
...
if ($Seco) {
    ...
    try {
        ...
        # para o Cargo nao refazer a compilacao e o LTO sem necessidade. Os
        # diretorios que o tauri-build gera nao contam como mudanca.
...
#
# ⚠️ **É a janela Tauri** (recordarfotos-e-commerce/docs/DESKTOP_TAURI.md), com
#    as telas do painel empacotadas. O app GPUI tem o próprio instalador
#    (`scripts/instalar-vintagelightbox-gpui.cmd`), e os dois convivem
#    instalados: nomes, pastas e caches diferentes.
#
...
NOME_LINUX="vintagelightbox-tauri"
...
O outro app, VintageLightbox (Zed GPUI), tem o próprio instalador:
...
if correr cargo build --release --manifest-path "$FONTE/Cargo.toml" -p app-tauri --bin app-tauri; then
  ...
  echo "   curl -fsSL https://raw.githubusercontent.com/alexkads/VintageLightbox/dev/scripts/instalar-vintagelightbox-tauri.cmd | CARGO_BUILD_JOBS=1 sh"
  exit "$CODIGO"
fi
