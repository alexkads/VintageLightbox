...
rem    Linux e macOS .. curl -fsSL https://raw.githubusercontent.com/alexkads/VintageLightbox/main/scripts/instalar-vintagelightbox-gpui.cmd | sh
rem
...
#   VLB_VERSAO (padrão main), VLB_DESTINO, VLB_SECO=1, VLB_SO_CONFERIR=1
# 🔑 **`main`, e não `dev`** (dono, 24/set/2026): o balcão é produção, e produção
#    só sai do `main`. O `dev` é onde o trabalho acontece; ele chega ao `main` por
#    `scripts/mains.sh atualizar`, no repositório do e-commerce, e só então aqui.
$Versao  = if ($env:VLB_VERSAO) { $env:VLB_VERSAO } else { "main" }
$Destino = if ($env:VLB_DESTINO) { $env:VLB_DESTINO } else { "$env:LOCALAPPDATA\Programs\VintageLightbox-GPUI" }
$Seco    = $env:VLB_SECO -eq "1"
# 🔧 **Só para o CI** (AppVeyor, 24/set/2026): prepara a máquina inteira, baixa o
#    código e roda `cargo check` em vez de `cargo build`, sem instalar nada. O
#    check roda os build.rs (shaders pelo fxc, LibRaw em C++) e confere todos os
#    tipos, mas nao gera nem liga o binario. A compilacao completa nao cabe nos
#    60 minutos do plano gratuito numa maquina de 2 nucleos; o balcao nunca liga isto.
$SoConferir = $env:VLB_SO_CONFERIR -eq "1"
...
$verbo = if ($SoConferir) { "check" } else { "build" }
$compilar = { cargo "+$Toolchain" $verbo --profile instalador --manifest-path (Join-Path $Fonte "Cargo.toml") -p ui-gpui --bin ui-gpui --target $Alvo }
try {
    Correr $compilar
} catch {
    if ($Seco) { throw }
...
if ($SoConferir) { Ok "cargo check passou: o codigo compila nesta maquina (nada foi instalado)"; return }
if ($LASTEXITCODE -ne 0 -or -not (Test-Path $binario)) {
    throw "a compilacao falhou, ou nao deixou $binario"
}

# ── Conferir o binario novo antes de tocar no instalado ──────────────────────
# 🔑 **O que funciona nao e tocado ate o novo provar que abre** (dono,
...
# 🔑 **Trocar sem corromper.** Com o app aberto, o Windows nao deixa
#    sobrescrever o .exe nem as DLLs que ele carregou, mas deixa renomear. O
#    instalado vira `.anterior`, o novo entra no lugar, e qualquer falha no
#    meio devolve os anteriores: o que funcionava continua funcionando.
$afastados = @()
function Afastar($arquivo) {
    if (-not (Test-Path $arquivo)) { return }
    $anterior = "$arquivo.anterior"
    Remove-Item $anterior -Force -ErrorAction SilentlyContinue
    if (Test-Path $anterior) { $anterior = "$arquivo.anterior-$(Get-Date -Format yyyyMMddHHmmss)" }
    Move-Item $arquivo $anterior -Force
    $script:afastados += ,@($arquivo, $anterior)
}
try {
    Afastar $exe
    Copy-Item $binario $exe -Force
    # O executavel liga com as DLLs do MinGW (libstdc++, libgcc, winpthread).
    # Fora do PATH do MSYS2 ele nao abriria, entao elas vao junto.
    foreach ($dll in "libstdc++-6.dll", "libgcc_s_seh-1.dll", "libwinpthread-1.dll") {
        $origem = Join-Path $mingw $dll
        if (Test-Path $origem) {
            Afastar (Join-Path $Destino $dll)
            Copy-Item $origem $Destino -Force
        }
    }
} catch {
    foreach ($par in $afastados) {
        Remove-Item $par[0] -Force -ErrorAction SilentlyContinue
        Move-Item $par[1] $par[0] -Force -ErrorAction SilentlyContinue
    }
    throw "a troca falhou e a versao anterior foi devolvida: $_"
}
...
#
#     --versao <ref>     branch ou tag a compilar. Padrão: main (produção)
...
# 🔑 `main`, e não `dev`: o balcão é produção (dono, 24/set/2026).
REF="main"; DESTINO=""; SECO=0; DIAGNOSTICO=0
...
Opções (com curl | sh, depois de \`sh -s --\`):
  --versao <ref>     branch ou tag. Padrão: main
...
if correr cargo build --profile instalador --manifest-path "$FONTE/Cargo.toml" -p ui-gpui --bin ui-gpui $RECURSOS; then
  ...
  echo "   curl -fsSL https://raw.githubusercontent.com/alexkads/VintageLightbox/main/scripts/instalar-vintagelightbox-gpui.cmd | CARGO_BUILD_JOBS=1 sh"
  exit "$CODIGO"
fi
...
# ── Conferir o binário novo antes de tocar no instalado ───────────────────────
# 🔑 **O que funciona não é tocado até o novo provar que abre** (dono,
#    2026-09-25: "não pode corromper a aplicação que já está funcionando").
#    `--versao` responde e sai antes de janela, catálogo e rede.
if [ "$SECO" -eq 0 ]; then
  diga "conferindo o app novo"
  RESPOSTA="$("$BINARIO" --versao 2>/dev/null | head -1)"
  if [ -z "$RESPOSTA" ]; then
    erro "o app novo não abriu (--versao); o instalado continua como estava"
    exit 1
  fi
  echo "   o app novo responde a versão $RESPOSTA"
fi
...
PLIST
  ...
  mkdir -p "$DESTINO"
  # 🔑 **Trocar sem corromper.** O novo vai ao lado com outro nome; o
  #    instalado vira `.anterior`; o novo entra por renomeação. Se algo falhar
  #    no meio, o anterior volta ao lugar.
  NOVO_APP="$DESTINO/.$NOME.app.novo"
  rm -rf "$NOVO_APP"
  ditto "$APP" "$NOVO_APP" || { rm -rf "$NOVO_APP"; erro "não consegui copiar o app novo; o instalado continua como estava"; exit 1; }
  rm -rf "$DESTINO/$NOME.app.anterior"
  if [ -d "$DESTINO/$NOME.app" ]; then
    mv "$DESTINO/$NOME.app" "$DESTINO/$NOME.app.anterior" \
      || { rm -rf "$NOVO_APP"; erro "não consegui afastar o app instalado; ele continua como estava"; exit 1; }
  fi
  if ! mv "$NOVO_APP" "$DESTINO/$NOME.app"; then
    [ -d "$DESTINO/$NOME.app.anterior" ] && mv "$DESTINO/$NOME.app.anterior" "$DESTINO/$NOME.app"
    erro "a troca falhou e a versão anterior foi devolvida"
    exit 1
  fi
  ...
  # 🔑 **Trocar sem corromper.** O novo é copiado ao lado; o instalado fica
  #    como `.anterior`; o novo entra por `mv`, que é atômico no mesmo disco —
  #    não há instante sem app. O app aberto segue com o arquivo antigo.
  NOVO_BIN="$DESTINO/bin/.$NOME_LINUX.novo"
  install -m 755 "$BINARIO" "$NOVO_BIN" \
    || { rm -f "$NOVO_BIN"; erro "não consegui copiar o app novo; o instalado continua como estava"; exit 1; }
  if [ -f "$DESTINO/bin/$NOME_LINUX" ]; then
    cp -p "$DESTINO/bin/$NOME_LINUX" "$DESTINO/bin/$NOME_LINUX.anterior" 2>/dev/null || true
  fi
  mv -f "$NOVO_BIN" "$DESTINO/bin/$NOME_LINUX" \
    || { rm -f "$NOVO_BIN"; erro "a troca falhou; o instalado continua como estava"; exit 1; }
