...
VERSAO="$VERSAO-$(shasum -a 256 < "$PUBLICO/conversao_web.js" | cut -c1-8)$(shasum -a 256 < "$PUBLICO/conversao_web_bg.wasm" | cut -c1-8)"
echo "$VERSAO" > "$PUBLICO/VERSAO"
cat > "$FONTE/versao.ts" <<TS
// Gerado por scripts/construir-web.sh do VintageLightbox-Rust — não editar.
// É o commit e o hash do conteúdo do motor que está em public/conversao/, e vai na URL dos dois
