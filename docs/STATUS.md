...
| 🚨 **No Windows, o handoff abria o Explorador de Arquivos, não o navegador** (achado do dono, 17/set/2026) | `abrir_no_navegador` chamava `explorer.exe <url>`. `explorer` é o **shell** do sistema: quando não reconhece o argumento como endereço ele não recusa — abre uma pasta. E `spawn().is_ok()` dava `true` nos dois desfechos, então nem a frase "abra este endereço à mão" aparecia. Agora é `open::that_detached` com a feature `shellexecute-on-windows`, que no Windows é a `ShellExecuteExW` com o verbo padrão (a mesma do duplo clique), e um endereço sem `http://`/`https://` é recusado antes de chegar ao shell (`e_endereco_web`, com teste). A feature se unifica com o `tauri-plugin-opener`, então os links de fora da janela (`app-tauri/src/navegacao.rs`) seguem o mesmo caminho |
