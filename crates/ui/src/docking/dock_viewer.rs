...
use crate::components::{
    advanced_slider::AdvancedSlider, color_labels::ColorLabels, crop_panel::CropPanel,
    filmstrip::Filmstrip, folder_tree::FolderTree, histogram::Histogram, image_viewer::ImageViewer,
    metadata_charts::MetadataCharts, photo_grid::PhotoGrid, rating_widget::RatingWidget,
    ...
    fn ui(&mut self, ui: &mut Ui, tab: &mut Self::Tab) {
        match tab {
            DockTab::PhotoGrid => {
                self.context
                    .photo_grid
                    .show(ui, self.context.state, self.context.ctx);
            }
            ...
            DockTab::Filmstrip => {
                ...
                let action = self
                    .context
                    .filmstrip
                    .show(ui, self.context.ctx, self.context.state);
                    ...
                    match action {
                        FilmstripAction::OpenInDevelop => {
                            if let Some(photo_id) =
                                self.context.state.library_selected_photo_id.clone()
                            {
                        ...
                        FilmstripAction::Export => {
                            ...
                            if let Some(metadata) = &self.context.state.detail_metadata.clone() {
                                ...
                                self.context.state.import_dialog_mode =
                                    crate::state::ImportDialogMode::Export;
                            } else if let Some(photo_id) =
                                &self.context.state.library_selected_photo_id
                            {
...
impl<'a> DockViewer<'a> {
    ...
    fn render_grid_settings(&mut self, ui: &mut Ui) {
        ...
        ui.horizontal(|ui| {
            for cols in [1, 2, 3, 4, 5] {
                ...
                if ui
                    .selectable_label(self.context.state.grid_columns == cols, label)
                    .clicked()
                {
    ...
    fn render_basic_adjustments(&mut self, ui: &mut Ui) {
        egui::ScrollArea::vertical().show(ui, |ui| {
            ...
            if ui
                .add(
                    egui::Slider::new(&mut self.context.state.active_exposure, -5.0..=5.0)
                        .show_value(false),
                )
                .changed()
            {
            ...
            if ui
                .add(
                    egui::Slider::new(&mut self.context.state.active_contrast, 0.0..=2.0)
                        .show_value(false),
                )
                .changed()
            {
            ...
            if ui
                .add(
                    egui::Slider::new(&mut self.context.state.active_temperature, -10.0..=10.0)
                        .show_value(false),
                )
                .changed()
            {
            ...
            if ui
                .add(
                    egui::Slider::new(&mut self.context.state.active_tint, -10.0..=10.0)
                        .show_value(false),
                )
                .changed()
            {
            ...
            if ui
                .add(
                    egui::Slider::new(&mut self.context.state.active_highlights, -100.0..=100.0)
                        .show_value(false),
                )
                .changed()
            {
            ...
            if ui
                .add(
                    egui::Slider::new(&mut self.context.state.active_shadows, -100.0..=100.0)
                        .show_value(false),
                )
                .changed()
            {
            ...
            if ui
                .add(
                    egui::Slider::new(&mut self.context.state.active_whites, -100.0..=100.0)
                        .show_value(false),
                )
                .changed()
            {
            ...
            if ui
                .add(
                    egui::Slider::new(&mut self.context.state.active_blacks, -100.0..=100.0)
                        .show_value(false),
                )
                .changed()
            {
            ...
            if ui
                .add(
                    egui::Slider::new(&mut self.context.state.active_clarity, -1.0..=1.0)
                        .show_value(false),
                )
                .changed()
            {
            ...
            if ui
                .add(
                    egui::Slider::new(&mut self.context.state.active_vibrance, -1.0..=1.0)
                        .show_value(false),
                )
                .changed()
            {
            ...
            if ui
                .add(
                    egui::Slider::new(&mut self.context.state.active_saturation, -1.0..=1.0)
                        .show_value(false),
                )
                .changed()
            {
            ...
            let export_label =
                format!("{} Export JPEG", crate::design_system::icons::ACTION_EXPORT);
    ...
    fn apply_preset(&mut self, preset: &domain::entities::Preset) {
        if let Some(v) = preset.adjustments.exposure {
            self.context.state.active_exposure = v;
        }
        if let Some(v) = preset.adjustments.contrast {
            self.context.state.active_contrast = v;
        }
        if let Some(v) = preset.adjustments.temperature {
            self.context.state.active_temperature = v;
        }
        if let Some(v) = preset.adjustments.tint {
            self.context.state.active_tint = v;
        }
        if let Some(v) = preset.adjustments.highlights {
            self.context.state.active_highlights = v;
        }
        if let Some(v) = preset.adjustments.shadows {
            self.context.state.active_shadows = v;
        }
        if let Some(v) = preset.adjustments.whites {
            self.context.state.active_whites = v;
        }
        if let Some(v) = preset.adjustments.blacks {
            self.context.state.active_blacks = v;
        }
        if let Some(v) = preset.adjustments.clarity {
            self.context.state.active_clarity = v;
        }
        if let Some(v) = preset.adjustments.vibrance {
            self.context.state.active_vibrance = v;
        }
        if let Some(v) = preset.adjustments.saturation {
            self.context.state.active_saturation = v;
        }
        if let Some(v) = preset.adjustments.tone_curve_shadows {
            self.context.state.active_tone_curve_shadows = v;
        }
        if let Some(v) = preset.adjustments.tone_curve_darks {
            self.context.state.active_tone_curve_darks = v;
        }
        if let Some(v) = preset.adjustments.tone_curve_lights {
            self.context.state.active_tone_curve_lights = v;
        }
        if let Some(v) = preset.adjustments.tone_curve_highlights {
            self.context.state.active_tone_curve_highlights = v;
        }
    ...
    fn render_hsl_color(&mut self, ui: &mut Ui) {
        egui::ScrollArea::vertical().show(ui, |ui| {
            ...
            for (name, value) in colors {
                ...
                if ui
                    .add(egui::Slider::new(value, -100.0..=100.0).show_value(false))
                    .changed()
                {
                    self.context.state.last_slider_change_time = Some(std::time::Instant::now());
                    self.context.state.pending_auto_save = true;
    ...
    fn render_hsl_hue(&mut self, ui: &mut Ui) {
        egui::ScrollArea::vertical().show(ui, |ui| {
            ...
            for (name, value) in colors {
                ...
                if ui
                    .add(egui::Slider::new(value, -180.0..=180.0).show_value(false))
                    .changed()
                {
                    self.context.state.last_slider_change_time = Some(std::time::Instant::now());
                    self.context.state.pending_auto_save = true;
    ...
    fn render_lens_corrections(&mut self, ui: &mut Ui) {
        egui::ScrollArea::vertical().show(ui, |ui| {
            ...
            if ui
                .add(
                    egui::Slider::new(
                        &mut self.context.state.active_lens_distortion,
                        -100.0..=100.0,
                    )
                    .show_value(false),
                )
            ...
            ui.horizontal(|ui| {
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    ui.label(format!(
                        "{:+.0}",
                        self.context.state.active_lens_vignette_amount
                    ));
                });
            });
            if ui
                .add(
                    egui::Slider::new(
                        &mut self.context.state.active_lens_vignette_amount,
                        -100.0..=100.0,
                    )
                    .show_value(false),
                )
            ...
            ui.horizontal(|ui| {
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    ui.label(format!(
                        "{:.0}",
                        self.context.state.active_lens_vignette_midpoint
                    ));
                });
            });
            if ui
                .add(
                    egui::Slider::new(
                        &mut self.context.state.active_lens_vignette_midpoint,
                        0.0..=100.0,
                    )
                    .show_value(false),
                )
    ...
    fn render_detail(&mut self, ui: &mut Ui) {
        egui::ScrollArea::vertical().show(ui, |ui| {
            ...
            if ui
                .add(
                    egui::Slider::new(&mut self.context.state.active_nr_luminance, 0.0..=100.0)
                        .show_value(false),
                )
                .changed()
            {
            ...
            if ui
                .add(
                    egui::Slider::new(&mut self.context.state.active_nr_color, 0.0..=100.0)
                        .show_value(false),
                )
                .changed()
            {
            ...
            if ui
                .add(
                    egui::Slider::new(&mut self.context.state.active_sharpen_amount, 0.0..=100.0)
                        .show_value(false),
                )
                .changed()
            {
            ...
            if ui
                .add(
                    egui::Slider::new(&mut self.context.state.active_sharpen_radius, 0.5..=3.0)
                        .show_value(false),
                )
                .changed()
            {
    ...
    fn render_all_adjustments(&mut self, ui: &mut Ui) {
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                ...
                let response =
                    egui::CollapsingHeader::new(egui::RichText::new("Crop & Straighten").strong())
                        .open(Some(is_open))
                        .show(ui, |ui| {
                            CropPanel::show(ui, self.context.state);
                        });
                ...
                ui.horizontal(|ui| {
                    let reset_label =
                        format!("{} Reset All", crate::design_system::icons::ACTION_RESET);
                    if ui.button(&reset_label).clicked() {
                        self.context.state.reset_edits();
                        self.context.state.pending_auto_save = true;
                        self.context.state.last_slider_change_time =
    ...
    fn render_basic_sliders(&mut self, ui: &mut Ui) {
        ...
        if AdvancedSlider::show(
            ui,
            "exposure",
            "Exposure",
            &mut self.context.state.active_exposure,
            -5.0..=5.0,
            0.0,
            2,
        ) {
        ...
        if AdvancedSlider::show(
            ...
            "contrast",
            "Contrast",
            &mut self.context.state.active_contrast,
            0.0..=2.0,
            1.0,
            2,
        ) {
        ...
        if AdvancedSlider::show(
            ...
            "temperature",
            "Temperature",
            &mut self.context.state.active_temperature,
            -10.0..=10.0,
            0.0,
            1,
        ) {
        ...
        if AdvancedSlider::show(
            ...
            "tint",
            "Tint",
            &mut self.context.state.active_tint,
            -10.0..=10.0,
            0.0,
            1,
        ) {
        ...
        if AdvancedSlider::show(
            ...
            "highlights",
            "Highlights",
            &mut self.context.state.active_highlights,
            -100.0..=100.0,
            0.0,
            0,
        ) {
        ...
        if AdvancedSlider::show(
            ...
            "shadows",
            "Shadows",
            &mut self.context.state.active_shadows,
            -100.0..=100.0,
            0.0,
            0,
        ) {
        ...
        if AdvancedSlider::show(
            ...
            "whites",
            "Whites",
            &mut self.context.state.active_whites,
            -100.0..=100.0,
            0.0,
            0,
        ) {
        ...
        if AdvancedSlider::show(
            ...
            "blacks",
            "Blacks",
            &mut self.context.state.active_blacks,
            -100.0..=100.0,
            0.0,
            0,
        ) {
        ...
        if AdvancedSlider::show(
            ...
            "clarity",
            "Clarity",
            &mut self.context.state.active_clarity,
            -1.0..=1.0,
            0.0,
            2,
        ) {
        ...
        if AdvancedSlider::show(
            ...
            "vibrance",
            "Vibrance",
            &mut self.context.state.active_vibrance,
            -1.0..=1.0,
            0.0,
            2,
        ) {
        ...
        if AdvancedSlider::show(
            ...
            "saturation",
            "Saturation",
            &mut self.context.state.active_saturation,
            -1.0..=1.0,
            0.0,
            2,
        ) {
    ...
    fn render_detail_sliders(&mut self, ui: &mut Ui) {
        ui.label(
            egui::RichText::new("Noise Reduction")
                .size(Theme::FONT_SM)
                .color(ui.visuals().weak_text_color()),
        );
        ...
        if AdvancedSlider::show(
            ...
            "nr_luminance",
            "Luminance",
            &mut self.context.state.active_nr_luminance,
            0.0..=100.0,
            0.0,
            0,
        ) {
        ...
        if AdvancedSlider::show(
            ...
            "nr_color",
            "Color",
            &mut self.context.state.active_nr_color,
            0.0..=100.0,
            0.0,
            0,
        ) {
        ...
        if AdvancedSlider::show(
            ...
            "sharpen_amount",
            "Amount",
            &mut self.context.state.active_sharpen_amount,
            0.0..=100.0,
            0.0,
            0,
        ) {
        ...
        if AdvancedSlider::show(
            ...
            "sharpen_radius",
            "Radius",
            &mut self.context.state.active_sharpen_radius,
            0.5..=3.0,
            1.0,
            1,
        ) {
