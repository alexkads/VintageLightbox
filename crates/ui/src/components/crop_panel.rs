...
impl CropPanel {
        ...
        egui::ScrollArea::vertical().show(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    ui.label(
                        egui::RichText::new("Crop & Straighten")
                            .size(Theme::FONT_SM)
                            .strong(),
                    );
                });
            ...
            ui.horizontal(|ui| {
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    // Lock button
                    let lock_icon = if state.selected_aspect_ratio == AspectRatio::Free {
                        "🔓"
                    } else {
                        "🔒"
                    };
            ...
            if let Some(crop) = &mut state.crop_settings {
                ...
                if ui
                    .add(egui::Slider::new(&mut angle, -45.0..=45.0).show_value(false))
                    .changed()
                {
            ...
            ui.horizontal(|ui| {
                ...
                if ui
                    .button("Auto")
                    .on_hover_text("Auto-straighten (not implemented)")
                    .clicked()
                {
            ...
            ui.horizontal(|ui| {
                if ui
                    .button("↺ Reset")
                    .on_hover_text("Reset crop (Shift+R)")
                    .clicked()
                {
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    ...
                    if ui
                        .button("Cancel")
                        .on_hover_text("Exit without applying (Esc)")
                        .clicked()
                    {
                        state.crop_mode_active = false;
                    }
    ...
    pub fn enforce_aspect_ratio(state: &mut AppState) {
        let current_crop = state
            .crop_settings
            .clone()
            .unwrap_or(domain::value_objects::CropSettings::default());
        ...
        if target_ratio > 0.0 {
            ...
            state.crop_settings = Some(domain::value_objects::CropSettings::new(
                ...
                new_w_n,
                new_h_n,
                current_crop.rotation_90(),
                current_crop.angle(),
                current_crop.flip_horizontal(),
                current_crop.flip_vertical(),
            ));
...
mod tests {
    ...
    fn test_enforce_aspect_ratio_square_selection_on_landscape_image() {
        ...
        let crop = state.crop_settings.unwrap();
        assert!(
            (crop.crop_width() - 0.5).abs() < 0.001,
            "Width should be 0.5, got {}",
            crop.crop_width()
        );
        assert!(
            (crop.crop_height() - 1.0).abs() < 0.001,
    ...
    fn test_enforce_aspect_ratio_tall_selection_on_landscape_image() {
        ...
        let crop = state.crop_settings.unwrap();
        assert!(
            (crop.crop_width() - 0.375).abs() < 0.001,
            "Width should be 0.375, got {}",
            crop.crop_width()
        );
        assert!(
            (crop.crop_height() - 1.0).abs() < 0.001,
