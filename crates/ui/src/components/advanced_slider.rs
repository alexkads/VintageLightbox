...
use egui::{pos2, Color32, CornerRadius, Rect, RichText, Sense, Stroke, TextEdit, Ui, Vec2};
...
impl AdvancedSlider {
    ...
    pub fn show(
        ...
        let mut is_editing =
            ui.memory(|mem| mem.data.get_temp::<bool>(edit_mode_id).unwrap_or(false));
        let mut edit_string = ui.memory(|mem| {
            mem.data
                .get_temp::<String>(id)
                .unwrap_or_else(|| format!("{:.prec$}", *value, prec = decimals))
        });
        ...
        ui.horizontal(|ui| {
            ...
            let label_response = ui.add(
                egui::Label::new(RichText::new(label).size(Theme::FONT_SM).color(label_color))
                    .sense(Sense::click()),
            );
        ...
        let rail_rect = Rect::from_min_size(
            pos2(
                rect.min.x + Theme::SPACE_SM,
                rect.center().y - Self::RAIL_HEIGHT / 2.0,
            ),
        ...
        if response.dragged() {
            ...
            if let Some(pos) = pointer_pos {
                let new_normalized =
                    ((pos.x - rail_rect.left()) / rail_rect.width()).clamp(0.0, 1.0);
