...
impl Histogram {
    ...
    pub fn show(ui: &mut Ui, histogram_data: Option<&HistogramData>) {
        ui.vertical(|ui| {
            ...
            if let Some(data) = histogram_data {
                ...
                ui.painter()
                    .rect_filled(rect, Theme::RADIUS_SM, Color32::from_rgb(30, 30, 35));
                ui.painter().text(
                    rect.center(),
    ...
    fn render_histogram(ui: &mut Ui, data: &HistogramData) {
        ...
        let rect = Rect::from_min_size(available_rect.min, Vec2::new(width, height));

        // Background
        ui.painter()
            .rect_filled(rect, Theme::RADIUS_SM, Color32::from_rgb(20, 20, 25));
