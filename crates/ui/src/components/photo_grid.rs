...
impl PhotoGrid {
    ...
    pub fn show(&mut self, ui: &mut Ui, state: &mut AppState, ctx: &egui::Context) {
        ...
        for result in results {
            ...
            let processed_image =
                if let Some(photo) = state.photos.iter().find(|p| p.id == result.photo_id) {
                    ...
                    let processed = if has_edits {
                        crate::image_processing::ImageProcessor::process_image(
                            ...
                            0.0,
                            0.0,
                            0.0, // tone curve (not applied to thumbnails)
                            0.0,
                            0.0,
                            0.0,
                            0.0,
            ...
            let resized_image =
                crate::image_processing::ImageProcessor::resize_for_preview(&processed_image, 300);
    ...
    fn show_tile_with_height(
        ...
        if photo.rating > 0 || response.hovered() {
            ...
            if response.hovered() {
                ui.painter()
                    .rect_filled(overlay_rect, 10.0, Color32::from_black_alpha(100));
            }
        ...
        if let Some(flag) = photo.flag {
            if flag != 0 {
                ...
                // Background circle
                let flag_color = if flag == 1 {
                    Theme::ACCENT_SUCCESS
                } else {
                    Theme::ACCENT_ERROR
                };
                ui.painter()
                    .circle_filled(flag_pos + Vec2::new(10.0, 10.0), 12.0, flag_color);
        ...
        if is_multi_selected && state.selection_count() > 1 {
            ...
            ui.painter()
                .circle_filled(check_pos, 10.0, Theme::ACCENT_PRIMARY);
            ui.painter().text(
                check_pos,
                egui::Align2::CENTER_CENTER,
                "✓",
