...
use domain::services::{
    FileOrganizer, ImageExporter, MetadataExtractor, PreviewStorage, PreviewType, SourceScanner,
    ThumbnailGenerator,
};
use domain::value_objects::{
    CropSettings, FilePath, OrganizationStrategy, PhotoId, PhotoMetadata, RenamePattern,
};
...
impl ImageExporter for DummyExporter {
    async fn export(&self, _photo: &domain::entities::Photo, _path: &FilePath) -> DomainResult<()> {
        Ok(())
    }
}
...
impl ThumbnailGenerator for DummyThumbnailGenerator {
    ...
    async fn generate_set(&self, _path: &FilePath, _sizes: &[u32]) -> DomainResult<Vec<Vec<u8>>> {
        Ok(vec![vec![]; _sizes.len()])
    }
}

struct DummyPreviewStorage;
impl PreviewStorage for DummyPreviewStorage {
    fn save(&self, _id: &PhotoId, _type: PreviewType, _data: &[u8]) -> DomainResult<()> {
        Ok(())
    }
    fn get(&self, _id: &PhotoId, _type: PreviewType) -> DomainResult<Option<Vec<u8>>> {
        Ok(None)
    }
    fn has(&self, _id: &PhotoId, _type: PreviewType) -> DomainResult<bool> {
        Ok(false)
    }
...
async fn setup_harness() -> (
    ...
    // Export with Dummy
    let export_uc = use_cases::ExportPhotoUseCase::new(photo_repo.clone(), Arc::new(DummyExporter));
    ...
    let get_sources_uc = Arc::new(use_cases::GetImportSourcesUseCase::new(Arc::new(
        DummyDeviceRepository,
    )));

    let scan_source_uc = Arc::new(use_cases::ScanSourceUseCase::new(Arc::new(
        DummySourceScanner,
    )));
    let describe_uc = Arc::new(use_cases::DescribeCandidatesUseCase::new(
        meta_extractor.clone(),
    ));
...
async fn test_keyboard_nav_saves_crop() {
    ...
    assert_eq!(
        ...
        Some("p2".to_string()),
        "Should navigate to p2"
    );

    // 2. saved_crop_settings should be set (This is our flag that the logic ran)
    assert!(
        state.saved_crop_settings.is_some(),
        "saved_crop_settings should be updated"
    );
    // Verify using getter
