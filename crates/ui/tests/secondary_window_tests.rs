...
mod tests {
        ...
        for i in 0..1000 {
            ...
            if i % 2 == 0 {
                ...
                assert_eq!(
                    window.session_id,
                    previous_session + 1,
                    "Session ID should increment on open"
                );
            } else {
                ...
                assert_eq!(
                    window.session_id, previous_session,
                    "Session ID should not increment on close"
                );
            }
        }
    }
    ...
    fn test_esc_signal_handling() {
        ...
        let mut harness = Harness::new_ui(|ui| {
            ...
            assert!(
                !window.is_open,
                "Window should be closed after processing signal"
            );

            // Assert signal is REMOVED to prevent re-triggering
            let signal_still_exists: bool =
                ui.ctx().data(|d| d.get_temp(close_req_id).unwrap_or(false));
            assert!(
                !signal_still_exists,
                "Signal should be consumed and removed"
            );
        });
    ...
    fn test_info_toggle_signal_handling() {
        let mut harness = Harness::new_ui(|ui| {
            ...
            assert!(
                window.show_info_overlay,
                "Info overlay should be ON initially"
            );
            ...
            assert!(
                !ui.ctx()
                    .data(|d| d.get_temp::<bool>(toggle_id).unwrap_or(false)),
    ...
    fn test_consume_esc_key() {
        let mut harness = Harness::new_ui(|ui| {
            ...
            if ui
                .ctx()
                .data(|d| d.get_temp::<bool>(close_req).unwrap_or(false))
            {
