...
async fn test_crop_mode_toggle_with_r_key() {
    ...
    assert!(state.crop_mode_active, "Crop mode should be active after R key");
    assert!(state.crop_settings.is_some(), "Crop settings should be initialized");

    // Toggle off
    state.crop_mode_active = !state.crop_mode_active;
    assert!(!state.crop_mode_active, "Crop mode should be inactive after second R key");
}
...
async fn test_aspect_ratio_selection() {
    ...
    assert_eq!(state.selected_aspect_ratio, domain::value_objects::AspectRatio::Original);

    // Change to Square
    state.selected_aspect_ratio = domain::value_objects::AspectRatio::Square;
    assert_eq!(state.selected_aspect_ratio, domain::value_objects::AspectRatio::Square);
...
async fn test_crop_rotation() {
    ...
    let crop = state.crop_settings.as_ref().unwrap();
    assert_eq!(crop.rotation_90(), 0, "Should be back to 0° after left rotation");
}
...
async fn test_crop_flip() {
    ...
    let crop = state.crop_settings.as_ref().unwrap();
    assert!(!crop.flip_horizontal(), "Should not be flipped horizontally");
    ...
    let crop = state.crop_settings.as_ref().unwrap();
    assert!(crop.flip_horizontal(), "Should still be flipped horizontally");
...
async fn test_crop_apply() {
    ...
    assert!(state.crop_settings.is_some(), "Settings should be preserved");
}
...
async fn test_crop_settings_validation() {
    ...
    assert_eq!(crop.crop_height(), 0.01, "Height < MIN should clamp to 0.01");
