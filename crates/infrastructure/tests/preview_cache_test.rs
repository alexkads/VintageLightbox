...
async fn test_preview_manager_cache_logic() {
    ...
    let thumb_bytes = generator.generate(&file_path, 300).await.expect("Failed to generate thumb");
    
    // Save to cache
    preview_manager.save(&photo_id, PreviewType::Thumbnail, &thumb_bytes).expect("Failed to save to cache");
    ...
    assert!(width == 100 || width == 300, "Width should be 100 (original) or 300 (scaled target), got {}", width); 
