...
//!
//! Algoritmos de processamento de imagem puros, sem dependência de framework UI.
...
impl ImageAlgorithms {
    ...
    /// # Arguments
    /// * `img` - Imagem original
    /// * `edits` - Parâmetros de edição a aplicar
    ///
    /// # Returns
    ...
    /// Esta é a função original movida da camada UI
    #[allow(clippy::too_many_arguments)]
    pub fn process_image_params(
        img: &DynamicImage,
        exposure: f32,
        contrast: f32,
        temperature: f32,
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                // 10. Vibrance (intelligent saturation)
                if vibrance != 0.0 {
                    let luminance = (r + g + b) / 3.0;
                ...
                if has_hsl_adjustment {
                    let (r_new, g_new, b_new) = Self::apply_hsl_saturation(
                        r, g, b,
    ...
    /// Aplica ajustes HSL de saturação por canal de cor
    #[allow(clippy::too_many_arguments)]
    fn apply_hsl_saturation(
        r: f32, g: f32, b: f32,
        hsl_red_sat: f32,
        hsl_orange_sat: f32,
        hsl_yellow_sat: f32,
    ...
    /// Redimensiona imagem para preview mantendo aspect ratio
    ...
    /// Aplica configurações de crop a uma imagem
...
mod tests {
    ...
    fn test_resize_for_preview() {
        ...
        let resized = ImageAlgorithms::resize_for_preview(&img, 1280);
    ...
    #[test]
    fn test_process_image_with_exposure() {
