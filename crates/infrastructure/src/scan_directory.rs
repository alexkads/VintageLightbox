...
impl ScanDirectoryUseCase {
    ...
    pub async fn execute(&self, directory_path: &Path) -> DomainResult<ScanDirectoryResult> {
        ...
        for file_path in files {
            match self.import_photo(&file_path, &exif_reader).await {
                ...
                Err(e) => {
                    failed.push((file_path.to_string_lossy().to_string(), e.to_string()));
                }
            }
        }
    ...
    async fn import_photo(
        &self,
        path: &Path,
        exif_reader: &crate::exif_reader::ExifReader,
    ) -> DomainResult<Photo> {
        ...
        let file_name = path.file_name().ok_or_else(|| {
            domain::DomainError::InfrastructureError("Invalid file name".to_string())
        })?;
...
mod tests {
    ...
    async fn test_scan_directory_success() {
        ...
        mock_repo.expect_save().returning(|_| Ok(())).times(..); // Allow any number of calls
    ...
    async fn test_scan_directory_with_failures() {
        ...
        mock_repo.expect_save().times(1).returning(|_| {
            Err(domain::DomainError::InvalidOperation(
    ...
    async fn test_scan_directory_recursive() {
        ...
        mock_repo.expect_save().returning(|_| Ok(())).times(..);
